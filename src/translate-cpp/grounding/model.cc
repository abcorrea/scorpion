...
public:
    ...
    */
    /*
      ...
      Emitted atoms go straight to `queue.push` (a non-virtual inline call,
      so the compiler can inline the whole push+dedup into each rule's empty
    ...
    */
    virtual void process(
        ...
        const vector<Atom> &items, AtomQueue &queue) = 0;
...
public:
    ...
    void process(
        ...
        AtomQueue &queue) override {
        auto eff_args = prepare_effect(new_atom, cond_index);
        queue.push(effect.predicate, move(eff_args));
    }
};
...
public:
    ...
    void process(
        const Atom &new_atom, int atom_index, int cond_index,
        const vector<Atom> &items, AtomQueue &queue) override {
        ...
        for (int stored_idx : it->second) {
            ...
            queue.push(effect.predicate, move(args));
        }
    }
};
...
public:
    ...
    void process(
        ...
        recurse(0, cond_index, eff_args, items, queue);
    }
...
private:
    ...
    void recurse(
        ...
        AtomQueue &queue) const {
        if (p == cond_index)
            ++p;
        if (p >= static_cast<int>(conditions.size())) {
            ArgList copy = args;
            queue.push(effect.predicate, move(copy));
            return;
        }
        ...
        for (int stored_idx : atoms_by_index[p]) {
            ...
            recurse(p + 1, cond_index, next_args, items, queue);
        }
    }
};
