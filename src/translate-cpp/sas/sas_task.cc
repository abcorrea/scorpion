...
void write_variables(FastWriter &w, const SASVariables &vars) {
    w.put(vars.ranges.size());
    w.nl();
    for (size_t i = 0; i < vars.ranges.size(); ++i) {
        w.put(string_view("begin_variable"));
        w.nl();
        w.put(string_view("var"));
        w.put(i);
        w.nl();
        w.put(vars.axiom_layers[i]);
        w.nl();
        w.put(vars.ranges[i]);
        w.nl();
        for (const auto &v : vars.value_names[i]) {
            w.put(v);
            w.nl();
        }
        w.put(string_view("end_variable"));
        w.nl();
    }
}

void write_mutex(FastWriter &w, const SASMutexGroup &m) {
    w.put(string_view("begin_mutex_group"));
    w.nl();
    w.put(m.facts.size());
    w.nl();
    ...
    w.put(string_view("end_mutex_group"));
    w.nl();
}

void write_init(FastWriter &w, const SASInit &init) {
    w.put(string_view("begin_state"));
    w.nl();
    for (int v : init.values) {
        w.put(v);
        w.nl();
    }
    w.put(string_view("end_state"));
    w.nl();
}

void write_goal(FastWriter &w, const SASGoal &goal) {
    w.put(string_view("begin_goal"));
    w.nl();
    w.put(goal.pairs.size());
    w.nl();
...
void write_operator(FastWriter &w, const SASOperator &op) {
    ...
    */
    w.put(string_view("begin_operator"));
    w.nl();
    w.put(clean);
    w.nl();
    w.put(op.prevail.size());
    w.nl();
    ...
    w.put(op.cost);
    w.nl();
    w.put(string_view("end_operator"));
    w.nl();
}
...
void write_axiom(FastWriter &w, const SASAxiom &ax) {
    ...
    w.put(ax.effect.first);
    w.put(' ');
    w.put(1 - ax.effect.second);
    w.put(' ');
    w.put(ax.effect.second);
    w.nl();
    w.put(string_view("end_rule"));
    w.nl();
}
}
...
void SASTask::output(ostream &os) const {
    ...
    w.put(string_view("begin_version"));
    w.nl();
    w.put(SAS_FILE_VERSION);
    w.nl();
    w.put(string_view("end_version"));
    w.nl();
    w.put(string_view("begin_metric"));
    w.nl();
    w.put(metric ? 1 : 0);
    w.nl();
    w.put(string_view("end_metric"));
    w.nl();
    ...
    for (auto &a : axs)
        ranges::sort(a.condition);
    ranges::sort(axs, [](const SASAxiom &a, const SASAxiom &b) {
